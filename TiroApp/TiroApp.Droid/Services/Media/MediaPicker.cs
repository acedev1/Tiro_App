    ...
    public class MediaPicker : IMediaPicker
    {
        ...
        private Intent CreateMediaIntent(int id, string type, string action, MediaStorageOptions options, bool tasked = true)
        {
            ...
            if (options != null)
            {
                ...
                {
                    pickerIntent.PutExtra(MediaPickerActivity.MaxPixelDimension, options.MaxPixelDimension.Value);
                }
                if (options.PercentQuality.HasValue)
                {
                    pickerIntent.PutExtra(MediaPickerActivity.PercentQuality, options.PercentQuality.Value);
                }
